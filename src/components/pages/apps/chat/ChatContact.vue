...
<template>
    <div class="chat__contact flex items-center px-2 pt-4 pb-2" :class="{'bg-primary-gradient text-white shadow-lg': isActiveChatUser}">
        <div class="contact__avatar mr-1">
            <vs-avatar v-if="contact.photoURL" class="border-2 border-solid border-white" :src="contact.photoURL" size="42px" />
            <vs-avatar v-else class="border-2 border-solid border-white" src="@/assets/images/profile/user-uploads/user-02.jpg" size="42px" />
        </div>
        <div class="contact__container w-full flex items-center justify-between overflow-hidden">
            <div class="contact__info flex flex-col truncate w-5/6">
                <h5 class="font-semibold" :class="{'text-white': isActiveChatUser}">{{ userLogged.username }}</h5>
...
export default {
  ...
  computed: {
    userLogged () {
      return this.$store.state.userLogged.user
    }
  }
}
</script>
