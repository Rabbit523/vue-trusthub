<template>
  <div class="parentx">
    <vs-sidebar
      ...
      :reduce-not-hover-expand="reduceNotRebound"
      :parent="parent"
    ...
    >
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        <!-- Header -->
        <div class="p-3 flex items-center justify-between" slot="header">
          <!-- Logo -->
          <router-link tag="div" class="vx-logo cursor-pointer mr-2 flex items-center" to="/">
            <img v-if="reduce" width="20" src="@/assets/images/logo/blogo.png" alt="Logo" />
            <template v-else>
              <div>
                <img
                  v-if="currentTheme !== 'light'"
                  class="dev-logo"
                  src="@/assets/images/logo/Trusthub_white_green.svg"
                  alt="Logo"
                />
                <img v-else class="dev-logo" src="@/assets/images/logo/Trusthub.svg" alt="Logo" />
              </div>
            </template>
          </router-link>
          <!-- /Logo -->
          ...
          <div>
            ...
            <template v-if="showCloseButton">
              <feather-icon
                icon="XIcon"
                class="m-0 cursor-pointer dev-close-white"
                @click="$store.commit('TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', false)"
              />
            </template>

            <!-- Toggle Buttons -->
            <template class="dev-pointer" v-else-if="!showCloseButton">
              <div @click="toggleReduce(!reduce)">
                <menu-icon :color="themePrimaryColor" height="50" class="dev-menu"></menu-icon>
              </div>
            </template>
          </div>
          <!-- /Menu Toggle Buttons -->
...
import MenuIcon from '../navbar/components/MenuIcon'
